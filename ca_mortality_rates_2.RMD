...
---
__Dataset__ is available from [healthdata.gov](https://chhs.data.ca.gov/Healthcare/California-Hospital-Inpatient-Mortality-Rates-and-/rpkf-ugbp). 
...
[Hospital Profitability for 2009-2013.](http://www.healthdata.gov/dataset/hospital-profitability-2009-2013)
...
__Question__: Can we predict hospital ratings based on risk adjusted mortality rates, number of deaths, number of cases and hospital profitability for 2012-2013?
...
__Load the data from the website.__ (not working)
```{r,warning=FALSE,message=FALSE,eval=FALSE}
fileUrl <- "http://oshpd.ca.gov/HID/Products/PatDischargeData/AHRQ/iqi/2013/iqi-imi-2013.html"
download.file(fileUrl,destfile="test_data.csv",mode="wb")
data <- read.csv("test_data.csv",sep=",",header=TRUE)
```
...
- The lower mortality rate, the bettwe hospital ratings are.
...
__Histrograms of # of Deaths, # of Cases and Risk.Adjusted.Mortality.Rate for Procedure.Condition.__ 
```{r,warning=FALSE,message=FALSE}
...
__Histrograms for Procedure.Condition.__ 
```{r,warning=FALSE,message=FALSE}
...
#table(df_clean$County,df_clean$Procedure.Condition)

#summary(df_clean$X..of.Cases)
#summary(df_clean$X..of.Deaths)
#summary(df_clean$Risk.Adjusted.Mortality.Rate)
```
...
__Mapping of Hospital.Ratings.__
```{r,warning=FALSE,message=FALSE}
