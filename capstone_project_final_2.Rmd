...
#### Hospital Ratings Prediction Using Random Forests for Dataset in __Long__ Format.
...
__Model performance__ on __train__ dataset using all variables.
```{r,warning=FALSE,message=FALSE}
# confusion matrix on train data
fit$confusion 
```

- Accuracy (how often is the classifier correct): `r 0.9537*100`
- Error Rate (how often is the classifier wrong): `r (1 - 0.9537)*100`

__Predictions__ on __test__ dataset using all variables. 
```{r,warning=FALSE,message=FALSE}
...
- Accuracy: `r 100*sum(diag(cm))/sum(cm)` 
- Error Rate: `r (1 - sum(diag(cm))/sum(cm))*100`
...
```{r,warning=FALSE,message=FALSE}
set.seed(34)
tree0 <- rpart(Hospital.Ratings ~ AcuteStroke + AMI + GIHemorrhage + HeartFailure + HipFracture, data = train_wide, method = "class",control=rpart.control(cp=0.001)) # cp determines when the splitting up of the decision tree stops
printcp(tree0)
...
c5_a <- sum(diag(cm))/sum(cm)
c5_e <- 1 - sum(diag(cm))/sum(cm)
```
...
__Condition variables: HeartFailure, AMI and GIHemorrhage are included in tree construction.__
```{r,warning=FALSE,message=FALSE}
tree1 <- rpart(Hospital.Ratings ~ HeartFailure + GIHemorrhage + AMI, data = train_wide, method = "class",control=rpart.control(cp=0.001)) # cp determines when the splitting up of the decision tree stops
printcp(tree1)
#plotcp(tree1)
...
- __Accuracy on the test data set using__
    ...
    - CART with five variables: `r round(c5_a,4)`
